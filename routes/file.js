...
router.get('/get/:id', passport.authenticate('bearer', { session: false }), (req, res, next) => {
  console.log('router.file.get');
  let includes = {
    id: [{ as: 'Tags', model: models.Tag }],
    all: [{ as: 'Tags', model: models.Tag }],
  };
  object.get('File', req.params.id, 1, includes)
    .then(response => {
